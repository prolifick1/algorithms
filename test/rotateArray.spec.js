...
describe('Rotate Array', function () {
  ...
  describe('Reversing linked list with positive number elements', function() {
   ...
   describe('Reversing linked list with negative number elements', function() {
    it('Input: nums = [-1,-100,-3,-99], k = 2, Output: [-3,-99,-1,-100]', () => {
      const nums = [-1, -100, -3, -99];
      expect((rotateArray(nums, 2), nums)).to.eql([-3, -99, -1, -100]);
    });
   });
  describe('Reversing linked list with negative rotation index', function() {
    xit('Input: nums = [1, 2, 3, 4, 5], k = -3, Output: [4, 5, 1, 2, 3]', function() {
      const nums = [1, 2, 3, 4, 5];
      expect((rotateArray(nums, -3), nums)).to.eql([4, 5, 1, 2, 3]);
    });
  });
});
